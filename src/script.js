...
async function handleLogin(email, password, rememberMe) {
    try {
        ...
        if (!res.ok) {
            ...
            showAuthError(loginModal, err.message || 'Login failed');
            return;
        }
        ...
        console.error(e);
        showAuthError(loginModal, 'Network error');
    }
}
...
async function handleRegister(email, password) {
    try {
        ...
        if (!res.ok) {
            const err = await res.json().catch(() => ({}));
            showAuthError(registerModal, err.message || 'Register failed');
            return;
        }
        ...
        console.error(e);
        showAuthError(registerModal, 'Network error');
    }
}
...
async function handleEditProfile(name, manualLat, manualLng, locationEnabled, emailNotify, phone) {
    ...
    currentUser.location_enabled = locationEnabled;
